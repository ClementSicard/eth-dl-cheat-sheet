...
\documentclass[a2paper,4pt]{extarticle}
...
``$\MX^2=\MX\MX^T$''$\geq 0$ ((symmetric) positive semi-definite)
...
2nd-Order Taylor expansion at $\vx_0$ (function for $\vx$, we want to
extrapolate to $f(\vx)$)
...
\subsection{Vect-by-Matrix (Gen. Gradient)}
...
Perceptron + Hopfield Network + MLP
...
\subsection{Comp. of Gradient via Backprop}
...
\begin{enumerate}
  ...
  \item Compute the gradient of $\cR$ w.r.t. the output layer activations (for a
...
\subsection{Special Case: Function Composition}
...
\subsection{Deep Function Compositions}

$
F_\theta\colon\cX=\R^n\to\R^m=\cY \qquad F_\theta=F^L_\theta\circ\cdots\circ F^1_\theta
$
...
\begin{enumerate}
  ...
  unit (subtraction) can represent any continuous p.w. linear function (exactly!)
  \item continuous p.w. linear function are dense in $C(\R^n)$
\end{enumerate}
...
\Def[$L$-Lipschitz Continuous Function] Given two \emph{metric spaces} $(X, d_X)$
...
In particular, a map $f\colon\R^n\to\R^m$ is called Lipschitz continuous if
...
compact subset $[a,b]\subset\R$ the function $e^x$ is Lipschitz continuous. On
...
\Thm[Conv. is Trans- (or Shift-) Invariant]
...
all the derivatives (note that some of them might be $0$).
...
\section{Recurrent Networks (RNNs)}
...
shift all the data points into one class. However, with BN since the mean is
...
\subsection{Regularization via Constrained Opti}
...
\subsubsection{Weight Rescaling}
...
a geometrically averaged ensemble (see DL-Book, 7.12).
...
\Def[Puzzling Capacity]: Ability of models to learn and represent complex patterns and relationships in large amounts of data. Puzzling = model's capacity to "figure out" or "uncover" the underlying structure in the data.

\Def[i.i.d assumption]: Past and future data are all sampled \textbf{independently} from the same distribution

\Def[Risk]: Let $\mathcal{F}$ be a function class. We search for $f_n \in \mathcal{F}$, which we call \textbf{hypothesis class}. Risk is then: 

$
R(f) = \Pr(f(x) \neq y) = \E_{\Pr}[\mathbbm{1}\{f(\vx)\neq y\}]
\approx \frac{1}{n}\sum_{i=1}^{n}\mathbbm{1}\{f(\vx)\neq y\}
$

By the law of large numbers, $\lim_{n\rightarrow\infty}R_n(f)=R(f)$

\Def[Empirical risk minimization (ERM)] is a learning objective approximating the target $f^*$ with the empirical risk minimizer $f_n$:
$
f_n = \argmin_{f\in\mathcal{F}}R_n(f)
$

\Def[Learning Theory task] is to produce bounds of the form $\Pr[R(f) \leq R_n(f) + \textbf{complexity term}] \geq (1 - \delta)$

\Thm[Hoeffding's bound]
$ \mathbb{P}\left[\left|\frac{1}{n} \sum_{i=1}^n Z_i-\mathbb{E}[Z]\right|>\epsilon\right] \leq 2 \exp \left(-\frac{2 n \epsilon^2}{(b-a)^2}\right) $

\Thm[Union bound] Let $C_1,...,C_N$ be a finite collection of $N$ events. Then:

$\Pr[C_1\cup...\cup C_N] \leq \sum_{i=1}^N\Pr[C_i]$

\subsubsection{VC Dimension}

\Def[Shattering coefficient]: Finite error bounds can also be obtained for infinite function classes. 

\textbf{Intuition}: Even infinite function classes have a finite modelling power $\rightarrow$ we need better ways to measure the effective size of a function class.

Consider the set $\mathcal{F}_{x_1,...,x_n} = \{(f(x_1),...,f(x_n)) : f\in \mathcal{F}\}$.
We define the \textbf{growth function} of $\mathcal{F}$ as: 

$\mathcal{S}_\mathcal{F}(n)=\sup_{x_1,...,x_n} \abs{\mathcal{F}_{x_1,...,x_n}}$

In short, $\mathcal{S}_\mathcal{F}(n)$ is the maximum number of ways into which $n$ points can be classified by the function class. It is also known as \textbf{shattering coefficient}

\Thm[A set of $n$ elements admits at most $2^n$ class.]

Thus $\mathcal{S}_\mathcal{F}(n) \leq 2^n$

\Def[VC Dimension] of a class $\mathcal{F}$ is defined as the size of the largest set that it can \textbf{shatter}, i.e. 

VC-dim$(\mathcal{F}) =$ largest $n$ for which $\mathcal{S}_\mathcal{F}(n)=2^n$

\Thm[VC inequality] ($\hat{\ell}$: empirical error, $\E[\ell]$: true (test) error)

$
\mathbb{P}\left\{\sup _{\mathcal{F}}|\hat{\ell}(f)-\E[\ell(f)]|>\epsilon\right\} \leq 8|\mathcal{S}_\mathcal{F}(n)| \exp{\frac{-s \epsilon^{2}}{32}}
$

\subsubsection{Double Descent}

\Def[Double Descent] is when test error of a model initially decreases due to learning, then increases with overfitting, then decreases again in the over-parameterized regime (as the capacity of the model grows). Moment of second descent is called \textbf{interpolation threshold}
...
\begin{itemize}
  ...
  \item Attention: selects what is relevant in retrospective
\end{itemize}
Both ideas can be combined!

\Def[Self-Gated Attention]: Attention-based transfer function for sequences (= self-attention + gating mechanism to control flow of info)

$\left.F\left(\boldsymbol{\xi},\left(\mathbf{h}_e^1, \ldots, \mathbf{h}_e^s\right)\right)=\left[
\mathbf{h}_e^1, \mathbf{h}_e^2,\ldots,\mathbf{h}_e^s\right] \cdot f_\phi\left(\boldsymbol{\xi},\left(\mathbf{h}_e^1, \ldots, \mathbf{h}_e^s\right)\right)\right)$

\Def[Self Attention] is an operation extensively used in the Transformer model. It helps to capture relations between words in a sentence, hence help to better encode the word. It maps a sequence of vectors $\vx_1,...,\vx_L$ with $\vx_i\in\mathbb{R}$ to a sequence of vectors $\vy_1,...,\vy_L$ by taking the weighted average of the input

$$
\vy_i = \sum_{j=1}^{L}\vw_{ij}\vx_{j}
$$

Here, $\vw_{ij}$ captures the interaction between individual input vectors $\vx_i$ and $\vx_j$. One possible way to capture this interaction is by using a normalized version of the inner product, i.e. $\vw_{ij}' = \vx_i^\top\vx_j$,

$$
\vw_{ij} = \frac{\exp{\vw_{ij}'}}{\sum_{j=1}^{L}\exp{\vw_{ij}'}} = \softmax(\vw_{ij}')
$$

Can be easily calculated with matrix multiplication: let $\MQ=\MX\MW^Q$, $\MK=\MX\MW^K$, and $\MV=\MX\MW^V$, then:

\begin{tightcenter}
$\MZ = \softmax\left( \frac{\MQ\MK^\top}{\sqrt{d_k}} \right) \MV$
\end{tightcenter}

with: 

\begin{itemize}
    \item $\MQ$: information about the word in relation to others
    \item $\MK$: how a word influences others 
    \item $\MV$: how a word sees itself
\end{itemize}

\subsubsection{Performance of Self-Attention}

\begin{itemize}
    \item \textbf{Self-Attention}: Comp. per layer: $\mathcal{O}(n^2d)$, Seq. operations: $\mathcal{O}(1)$, Max. Path length: $\mathcal{O}(1)$
    \item \textbf{RNN}: Comp. per layer: $\mathcal{O}(nd^2)$, Seq. operations: $\mathcal{O}(n)$, Max. Path length: $\mathcal{O}(n)$
    \item \textbf{CNN}: Comp. per layer: $\mathcal{O}(knd^2)$, Seq. operations: $\mathcal{O}(1)$, Max. Path length: $\mathcal{O}(\log_k(n))$
\end{itemize}

\Def[Multi-head attention] is a module for attention mechanisms which runs through an attention mechanism several times in parallel. The independent attention outputs are then concatenated and linearly transformed into the expected dimension. Intuitively, multiple attention heads allows for attending to parts of the sequence differently (e.g. longer-term dependencies versus shorter-term dependencies).

\sep
\subsection{BERT}

\begin{itemize}
    \item Mask-based training approach
    \item Only uses encoder stack of Transformers
    \item A token for whole sentence embedding used for classification tasks
    \item A separator token
\end{itemize}

Models based on Transformers: BERT (only encoding), GPT-x, DALL-E
...
very long sequence, it might have the issue that suddenly we have to store the
entire sequence in a single vector. But when we as humans translate we translate
small parts into small parts. How it works:

\begin{itemize}
  \item Attend to the hidden state sequence $(\vh_e^1,...,\vh_e^s)$ of the encoding RNN
  \item Decoding RNN produces query at each time, i.e. $\xi^1,...,\xi^t$
  \item Self-gated attention produces "read-outs" $(\vz^1,...,\vz^t)$
  \item Used as input to the decoding RNN: $(\vh_d^r\vz^r) \mapsto \vy_d^{r+1}$
\end{itemize}

\subsection{Transformer}
\Def[Transformers] is an encoder-decoder architecture. More precisely a stack of encoders and decoders.

Inside each encoder/decoder layer, we have the following structure (from bottom to top):
\begin{itemize}
    \item \textbf{Self-Attention}: to look at other words in the sentence
    \item \textbf{\textit{Encoder-Decoder Attention}} (only if Decoder block): to focus on part of the input sentence
    \item \textbf{Feed-forward}: Applied independently to each position
...
\sep
Attention learns to index, multiplicative gating to combine bottom-up and top-down information

\section{Geometric Deep Learning}

\subsection{Graph NNs}

\Def[Equivariance and invariance for graphs]: Permutation on the adjacency matrix is given by $\MP\MA\MP^\top$. For suitable functions $f(\MX, \MA)$ for data over graphs:
\begin{itemize}
    \item \textbf{Invariance}: $f(\MP\MX, \MP\MA\MP^\top) = f(\MX, \MA)$
    \item \textbf{Equivariance}: $f(\MP\MX, \MP\MA\MP^\top) = \MP f(\MX, \MA)$
\end{itemize}

\Def[3 types of problems for GNNs]: Node classification, graph classification and link prediction

\Def[Applications of GNNs]: Semi-supervised learning, recommender systems, molecule classification, 
...
\begin{itemize}
  ...
  $\vx\mapsto\vz=\vx^l$
  \item \emph{Decoder}: $H=F_L\circ\cdots\circ F_{l+1}\colon\R^m\to\R^n$,
  $\vz\mapsto\vy=\hat{\vx}$.
...
$\MU$ and $\MV$ are \textbf{orthogonal} - so we have an orthogonal
basis. Via the SVD we can get the best rank-$k$
approximation of a linear mapping. It also preserves as
...
\MV_m^\T
\]
where the $m$ refers to the matrices of the $m$-reduced SVD.

\ssep

\Cor This means that a linear auto-encoder with $m$ hidden units cannot improve
the SVD since $\rank(\MC\MD)\leq m$ but can achieve the same result.
...
Note: the solution is \emph{not unique!} For any
...
\MA^{-1}=\MA^\T
\]
hence $\MA\in O(m)$ (orthogonal group, rotation matrices). Then the mapping
$\vx\to\vz$ is determined (up some rotation that we do in-between, rotation and
its inverse).

\subsection{Principal Component Analysis}

A way to solve this problem is through PCA. 
First, we center the data (pre-processing) as follows:
\[
\vx_i
\mapsto
\vx_i
...
do the SVD of $\MS$. We can see that
\[
...
\subsubsection{Regularized, Non-Linear Autoencoders}

\Def[Non-Linear Autoencoder] contains non-linear layers with activation functions, 
and training can be done via MLE. (only requirement: bottleneck layer).
...
Autoencoders allow us to separate the signal from noise: De-noising
...
\norm{\vx-\vx_{\veta}}^2
\]
The reconstruction error of the noisy data is lower than the error introduced by the noise, indicating successful denoising.
...
% So we wanted the error to be the same for each of the
...
\subsection{Refresher on MGFs and Gaussians}
...
Every distribution has its unique kind of MGF form. Hence, MGFs can be very
